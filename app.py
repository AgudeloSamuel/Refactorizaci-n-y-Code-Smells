...
def agregar():
    ...
    for h in Horario.query.filter_by(salon=salon, dia=dia).all():
    ...
    for h in Horario.query.filter_by(profesor=profesor, dia=dia).all():
        ini = datetime.strptime(h.inicio, "%H:%M").time()
        fin_h = datetime.strptime(h.fin, "%H:%M").time()
        if inicio_nuevo < fin_h and fin_nuevo > ini:
...
def editar(id):
    ...
    if request.method == "POST":
        ...
        for h in Horario.query.filter_by(profesor=horario.profesor, dia=dia).all():
            if h.id == horario.id:
                continue
            ini = datetime.strptime(h.inicio, "%H:%M").time()
            fin_h = datetime.strptime(h.fin, "%H:%M").time()
